...
describe Essentia::Models::OptionSet do

  before :example do
    @runtime_opts = { 'this' => 1.0, 'that' => 23.0 }
    @default_opts = { 'this' => 10.0, 'that' => 23.0, 'the_other' => 'sixteen' }
    @os = Essentia::Models::OptionSet.new('test', @default_opts, @runtime_opts)
    @os_with_no_rh = Essentia::Models::OptionSet.new('test', @default_opts)
    @out_methods = [nil, :toReal, :toString]
  end

  it 'can be created with different arguments' do
    expect(@os).not_to be nil
    expect(@os.is_a?(Essentia::Models::OptionSet)).to be true
    expect(@os_with_no_rh).not_to be nil
    expect(@os_with_no_rh.is_a?(Essentia::Models::OptionSet)).to be true
  end

  it 'sets up the essentia parameters properly with runtime options' do
    osp_should_be = [ 'this', Essentia::Parameter.new(1.0), 'that', Essentia::Parameter.new(23.0), 'the_other', Essentia::Parameter.new('sixteen') ]
    @os.to_essentia_parameters.each_index do
      |idx|
      p = @os.to_essentia_parameters[idx]
      psb = osp_should_be[idx]
      value = p.is_a?(Essentia::Parameter) ? p.send(@out_methods[psb.type]) : p
      expected_value = psb.is_a?(Essentia::Parameter) ? psb.send(@out_methods[psb.type]) : psb
  ...
  it 'sets up the essentia parameters properly without runtime options' do
    ospnorh_should_be = [ 'this', Essentia::Parameter.new(10.0), 'that', Essentia::Parameter.new(23.0), 'the_other', Essentia::Parameter.new('sixteen') ]
  ...
  it 'sets up methods for each parameter (properly)' do
    ...
    @os.default_hash.each { |k, v| expect(@os.send(k)).to eq(Essentia::Parameter.new(v)) }
    @os_with_no_rh.default_hash.keys { |m| expect(@os_with_no_rh.respond_to?(m)).to be true }
    @os_with_no_rh.default_hash.each { |k, v| expect(@os_with_no_rh.send(k)).to eq(Essentia::Parameter.new(v)) }
  end

end

describe Essentia::Models::OptionManager do

  before :example do
    @runtime_opts = { 'this' => 1.0, 'that' => 23.0 }
    @default_opts = { 'this' => 10.0, 'that' => 23.0, 'the_other' => 'sixteen' }
    @os1 = Essentia::Models::OptionSet.new('test 1', @default_opts, @runtime_opts)
    @os2 = Essentia::Models::OptionSet.new('test 2', @default_opts, @runtime_opts)
    @out_methods = [nil, :toReal, :toString]
    @dh = { 'this' => 1.0 }
    @rh = { 'this' => 2.0 }
    @params = [ 'this', Essentia::Parameter.new(2.0) ]
  end

  it 'can be created (with no arguments)' do
    expect(Essentia::Models::OptionManager.new).not_to be nil
  end

  it 'can receive OptionSet objects and updates to them' do
    expect((om = Essentia::Models::OptionManager.new)).not_to be nil
  ...
  it 'has a handle_defaults() method that loads all option in the proper place' do
    ...
    expect(om['test 6'].is_a?(Essentia::Models::OptionSet)).to be true
    expect(om['test_7'].is_a?(Essentia::Models::OptionSet)).to be true
...
# 
# describe 'Essentia::Helpers::Option.handle' do
# 
#   before :example do
#     @h1 = { 'this' => 1.0 }
#     @h2 = { 'this' => 2.0 }
...
#     expect{ Essentia::Helpers::Option.handle }.to raise_error(ArgumentError, "arguments must be three or multiples of three")
#   end
# 
#   it 'raises an ArgumentError if called with a number of args that is not a multiple of three' do
#     expect{ Essentia::Helpers::Option.handle(1, 2, 3, 4) }.to raise_error(ArgumentError, "arguments must be multiples of three")
#   end
# 
#   it 'raises an ArgumentError if arguments are not a String or Symbol, Hash, Hash triplet' do
#     expect{ Essentia::Helpers::Option.handle(1, 2, 3) }.to raise_error(ArgumentError, 'arguments must be in the form: string (or symbol), hash, hash, ... etc.')
#     expect{ Essentia::Helpers::Option.handle('test 3', @h1, @h2, 1, 2, 3) }.to raise_error(ArgumentError, 'arguments must be in the form: string (or symbol), hash, hash, ... etc.')
#     expect{ Essentia::Helpers::Option.handle(1, 2, 3, 'test 4', @h1, @h2) }.to raise_error(ArgumentError, 'arguments must be in the form: string (or symbol), hash, hash, ... etc.')
#     expect{ Essentia::Helpers::Option.handle(:test_5, @h1, @h2, 1, 2, 3) }.to raise_error(ArgumentError, 'arguments must be in the form: string (or symbol), hash, hash, ... etc.')
#     expect{ Essentia::Helpers::Option.handle(1, 2, 3, :test_6, @h1, @h2) }.to raise_error(ArgumentError, 'arguments must be in the form: string (or symbol), hash, hash, ... etc.')
#   end
# 
#   it 'creates an OptionManager with all option sets in the proper place' do
#     expect((om = Essentia::Helpers::Option.handle('test 6', @h1, @h2, :test_7, @h1, @h2)).is_a?(Essentia::Models::OptionManager)).to be true
#     expect(om['test 6'].is_a?(Essentia::Models::OptionSet)).to be true
#     expect(om['test_7'].is_a?(Essentia::Models::OptionSet)).to be true
#     expect(om[:test_7]).to be nil
#     ['test 6', 'test_7'].each do
